...
const getRecaptures = async (req, res) => {
	...
	try {
		...
		if (errorCheck.recordsets[0][0] == null) {
			...
			res.json({ status: status.recordsets[0][0], error: "NONE", transfared: checkIfTransfer.recordset[0] })


		} else {
			res.json({ status: status.recordsets[0][0], error: errorCheck.recordsets[0][0] })
		}
